...
import { LogOut, Calendar, Users, Lock, ChevronLeft, ChevronRight, Phone, Mail, XCircle } from 'lucide-react'
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                className="input-field" placeholder="Enter admin password" required />
            </div>
            {loginErr && (
              <p className="text-red-600 text-sm flex items-center gap-1">
                <XCircle className="w-4 h-4" /> {loginErr}
              </p>
            )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setView('table')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'table' ? 'bg-brand-green text-white' : 'text-gray-300 hover:text-white'}`}>
              <Users className="w-4 h-4" /> Leads
            </button>
            <button onClick={() => setView('calendar')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'calendar' ? 'bg-brand-green text-white' : 'text-gray-300 hover:text-white'}`}>
              <Calendar className="w-4 h-4" /> Calendar
            </button>
            <button onClick={handleLogout}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-white/10 transition-colors ml-2">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-gray-500 uppercase text-xs tracking-wider">
                    <tr>
                      {['Name','Phone','Email','Service','Preferred Date','Time','Submitted','Status'].map(h => (
                      ...
                      <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <select value={lead.status} onChange={e => handleStatusChange(lead.id, e.target.value)}
                            className={`text-xs font-semibold px-2 py-1 rounded-full border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-green ${STATUS_STYLES[lead.status]}`}>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="grid grid-cols-7">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div key={day} className={`min-h-[80px] border-b border-r border-gray-100 p-2 ${isToday ? 'bg-green-50' : 'hover:bg-gray-50'} transition-colors`}>
                    <span className={`text-sm font-semibold inline-flex items-center justify-center w-7 h-7 rounded-full ${isToday ? 'bg-brand-green text-white' : 'text-gray-700'}`}>
                      {day}
                    </span>
